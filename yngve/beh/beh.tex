...
Given any $\Sigma$-algebra $A$ and an equivalence (not necessarily a
congruence) on $\Card A$, the multi-quotient construction yields a
multialgebra $\quo A$. Conversely,
\begin{Fact}
For any $\Sigma$-multialgebra $M$ there exists a $\Sigma$-algebra $A$ and an
equivalence such that $\quo A\iso M$.
\end{Fact}
\begin{Proof}
Have to take care of constants in some way -- treat them as unary operations?

For every $f\in\Sigma$ and $x,\ovr x\in\Card M$, let $R=f^M(x,\ovr x)$. If
cardinality of $R$ is $k>1$ 
\begin{ite}
\item add $k-1$ new elements $\{e_i:1\leq i<k\}$ to the carrier of sort of $x$
\item denote $x$ by $e_0$; make each $e_i\equiv_A e_0$
\item define $f^A(e_i,\ovr x)=r_i$, where $R=\{r_i:0\leq i<k\}$
\item for each other operation $g\in\Sigma$, let $g^A(...e_i...) =
g^M(...e_0...)$
\end{ite}
\com{Roughly, need to fix details a bit}
If $M$ allows functions returning empty set, $A$ must be partial!!!
\end{Proof}
Thus, a multialgebra $M$ is just a pair $\pair A\equiv$, where $\equiv$ is an
arbitrary equivalence (not necessarily a congruence) on $\Card A$.


\com{Characterize those multialgebras which do arise from observational
algebras}
When $\equiv$ is observational-congruence, we get ... a usual algebra. So,
there is no need for multi-...? But the idea is exactly that nondeterminism
somehow helps us...

\com{Do you want to consider determinizations}
\begin{Definition}
A $\Sigma$-algebra $A$ is a (complete) determinization of a
$\Sigma$-multialgebra $M$, $A\in\CDet(M)$, if $A$ is a (weak) subalgebra of $M$.
\end{Definition}
\section{Homomorphisms?} 
$\phi:\pair A{\equiv_A}\into \pair B{\equiv_{B}}$ should be
somethings which yield appropriate multialgebra homomorphisms when performed
on the respective quotients $\hat\phi:\quo A\into \quo B$.
Minimum requirement is, probably, preservation of $\equiv$, in order to be
able to define $\hat\phi([x]_A)\Def [\phi(x)]_B$.
...
\enddiagram
\]
\rem{The canonical model should move all nondeterminism to constants? Like
in $B$...\label{const}}
%
