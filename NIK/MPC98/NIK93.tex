...
\maketitle
\abstract{\noindent
The paper begins by arguing that nondeterminism should be 
distinguished
from underspecification since the latter does not provide sufficient means of 
abstraction. The need for a ``true'' nondeterminism is illustrated by an example
based on the use of unordered structures and iteration over such 
structures as sets. 
We then point out that nondeterminism subsumes underspecification and,
...
does not make the situation worse. For those who cannot part with the 
initial semantics we suggest, as a possible alternative, the 
quasi-initial semantics of nmondeterminism.
...
We begin with a few remarks on this issue and then proceed to with an example
...
operation $\ch:S\times S\into S$ which, for arguments $x,y$, should equal 
...
behave like choice and not like, for instance, yet another constructor. That is, we may have
...
a directed, irreflexive graph. We assume that a (connected) graph $g$ 
is represented as a set $G$ of edges, each edge being a pair 
$\<x,y\>$ of the source and target vertex. (The set $V$ of all 
vertices in a graph can be easily obtained from $G$.)
%as a pair $\<V,N\>$, where $V$ is a set of vertices and 
Given a graph $g$, the function $N:V\into\PSet(V)$  
returns, for every vertex $v$ the set of its neighbours in $g$. Working with sets, 
...
calls to operation $Tr:Gr\times V\times Gr\into Gr$, where the second 
$Gr$-argument is a set of edges 
representing the tree (graph) constructed so far.
\[\begin{array}{rrcl}
1.& DFS(g,v) & = & Tr(g,v,\es) \\
...
because all were visited (marked by removing them from $g$) or because $v$ is a leaf), 
$Tr$ backtracks (axiom 2).
If the set $N(v)$ is not empty, i.e., it is $S+x$ for some set $S$ 
and vertex $x$, there are two possibilities for each element of this 
set. 
...
graphs, in particular, the neighbours $N(v)$ to be a sequence. This, however, is
...
An abstract definition of $DFS$ should {\em not} assume any such 
particular representation of the sets of neighbours. It also should 
{\em not} force the implementor to construct
...
$\ch:S\into E$ (specified appropriately, for instance, by an axiom
$z\Incl \ch.(S+x)\Impl z\Eq x\lor z\Incl \ch.S$, or, perhaps, $S\Eq\es\lor\ch(S)\in S$.)
\[\begin{array}{rrcl}
...
\section{Initiality + Nondeterminism $\Impl$ Junk}\label{se:junk}
...
Horn clauses is not sufficient. Hu{\ss}mann \cite{c:59} specified other, sufficient (and slightly 
...
$\{\bot, \_\sqcap\_, \_\ch\_\}$ and binary predicates $\{=, \Incl, :\}$. 
...
interpreted as joins. Thus nondeterministic choice is here a predefined
...
We can get rid of some of such ``vacuous sorts'' by introducing additional axioms. 
...
then $a\ch c=a\meet c=a$).\footnote{We can give some plausibility to the presence of 
joins -- not knowing 
whether $c$ is $a$ or $b$, we do not know whether $a\ch c$ is going to be 
$a$ or $a\ch b$, i.e., we 
have to keep both. It is more difficult to give a natural interpretation of meets, in
...
As in the case of multialgebras, the removal of the axiom $c:a\ch b$ underspecifying $c$ would lead to 
the intended  initial model with the elements $\{\bot, a,b,a\ch b\}$.
...
without actually identifying $c$ with any of the two: the source of ``intuitive junk'' 
in all the 
...
different kinds of nondeterministic operations without 
...
Axiom 1., on the other hand, specifies binary choice by underspecifying each particular {\em application} 
...
If for every ground term $t$, there is a deterministic ground term $s$ such that 
...
``either ... or ...'' \cite{c:137}. For $SPN2$, such a semantics can be summarized by saying 
...
at least, both $a$ and $b$. The quasi-initial semantics of $SPN2$ is given by the 
