...
\multicolumn{3}{|c|}{$x\Incl y,\GSD,x=y$
\ \ \ \ \ \ \ $x=y,\GSD,x\Incl y$} \\[2ex]
%
...
antecednet, but they all have a variable in the LHS, % $x\Incl t,\GSD,x\Incl t$,
...
We will treat these two rules quite similarly and refer to them jointly as ($\preceq_{sx}$). Elimination of these rules was the motivation for the inclusion of the
additional axioms in $NEQ_4$.
\begin{LEMMA}\label{le:noInclsx}
 $\der{NEQ_{3}}DS\ \impl\ \der{NEQ_{3}}{D'}S$ and $D'$ contains no applications
 of ($\preceq_{sx}$). 
%% not any more [Also $\#(\preceq_{anr},D')=0$ but this isn't crucial.]
%\leq\#(\preceq_{anr},D)$.
...
%% \item $\GSD,t\Incl t$
%% \[ \begin{array}{rl}
%%  x=s, \GSD, t(s)\Incl t(s) \\ \cline{1-1}
%%  x=s, \GSD, t(x) \Incl t(s) & \rabove{(=_{sx})}
%% \end{array} 
%% \conv
%% \begin{array}{rl}
%%  t(x)\Incl t(s),x=s,\GSD, t(s)\Incl t(s) \\ \cline{1-1}
%%  t(x)\Incl t(s),x=s,\GSD, t(x) \Incl t(s) & \rabove{(\Incl_{s})} \\ \cline{1-1}
%%  t(s)\Incl t(s),x=s,\GSD, t(x) \Incl t(s) & \rabove{(=_{Lar})} \\ \cline{1-1}
%%  x=s, \GSD, t(x) \Incl t(s) & \rabove{(cut_t)}
%% \end{array}
%% \]
%% This works for the situation when $t(s)\not= s$, since then
%% in the application ($\Incl_s$) the active formula is $t(x)\Incl t(s)$. 
%% If $t(s)=s$, and so $t(x)=x$, we have
%% \[ \begin{array}{rl}
%%  x= y, \GSD, y\Incl y \\ \cline{1-1}
%%  x=y, \GSD, x \Incl y & \rabove{(=_{sx})}
%% \end{array} 
%% \conv
%% \begin{array}{rl}
%%  x= y, \GSD, x \Incl y & axiom
%% \end{array}
%% \]
%% For the rule ($\Incl_{sx}$) we have similar cases
%% \[ \begin{array}{rl}
%%  x\Incl s, \GSD, t(s)\Incl t(s) \\ \cline{1-1}
%%  x\Incl s, \GSD, t(x) \Incl t(s) & \rabove{(\Incl_{sx})}
%% \end{array} 
%% \conv
%% \begin{array}{rl}
%%  t(x)\Incl t(s),x\Incl s,\GSD, t(s)\Incl t(s) \\ \cline{1-1}
%%  t(x)\Incl t(s),x\Incl s,\GSD, t(x) \Incl t(s) & \rabove{(\Incl_{s})} \\ \cline{1-1}
%%  t(s)\Incl t(s),x\Incl s,\GSD, t(x) \Incl t(s) & \rabove{(\Incl_{ar})} \\ \cline{1-1} x\Incl s, \GSD, t(x) \Incl t(s) & \rabove{(cut_t)}
%% \end{array}
%% \]
%% In the application ($\Incl_s$) the active formula is $t(x)\Incl t(s)$. If $t(s)=s$, 
%% i.e. $t(x)=x$ we get:
%% \[ \begin{array}{rl}
%%  x\Incl y, \GSD, y\Incl y \\ \cline{1-1}
%%  x\Incl y, \GSD, x \Incl y & \rabove{(\Incl_{sx})}
%% \end{array} 
%% \conv
%% \begin{array}{rl}
%%  x\Incl y, \GSD, x \Incl y & axiom
%% \end{array}
%% \]
...
\end{array}
\]
%% \item $x\Incl y,\GSD,y\Incl x$ -- for ($=_{sx}$) we have two possibilities:
%% \[
%% \prar{
%% y=z, x\Incl y,\GSD, y\Incl x \cl
%% y=z, x\Incl y,\GSD, z\Incl x & \rabove{(=_{sx})}
%% }
%% \conv
%% \prar{
%% x=z, x\Incl y,\GSD, z\Incl x \cl
%% y=z, x\Incl y,\GSD, z\Incl x & \rabove{(\Incl_{ar})}
%% \]
%% \[
%% \prar{
%% x=z, x\Incl y,\GSD,y\Incl x \cl
%% x=z, x\Incl y,\GSD,y\Incl z & \rabove{(=_{sx})}
%% }
%% \conv
%% \prar{
%% z\Incl y, x=z, x\Incl y,\GSD,y\Incl z \cl
%% x\Incl y, x=z, x\Incl y,\GSD,y\Incl z & \rabove{(=_{Lar})}
%% }
%% \]
%%!! does not go for \Incl_{sx} !!
\end{LS}
...
\PROOFRULE{s\Incl t,\GSD,p\Incl f(s)}{s\Incl t,\GSD,p\Incl f(t)} ($\Rsr$)\ \ \ \ 
$f(t)\not\in\Vars$
\end{LEMMA}
\begin{PROOF}
...
according to the lemmas \ref{le:noeqeq} through \ref{le:noeqSD}, %{le:noInclsx},
...
%%? \item the rules ($=_{Lar}$) and ($\Incl_{ar}$) restricted by the additional 
%%? requirement that $w(t)\not\in\Vars$, and
\item
with the simple cut rules \reff{ru:sicut}, \reff{ru:cutxeq} 
...
%% \multicolumn{3}{|c|}{$x\Incl y,\GSD,x=y$
%% \ \ \ \ \  $x=y,\GSD,x\Incl y$
%% \ \ \ \ \  $x\Incl y,\GSD,y\Incl x$} \\[2ex]
%%?
%%?\multicolumn{3}{|c|}{$\Gamma\Seq\Delta,p\Incl p$;\ \ \ \ \ \ 
%%?$\Gamma\Seq\Delta, x=x$;\ \ \ \ \ \ 
...
%% \Gamma\Seq\Delta}{x=t, w(t)\Incl q,\Gamma\Seq\Delta} ($=_{Lar}$) & \\[3ex]
%%? &  \multicolumn{2}{l|}{{\footnotesize \ \ \ - $w(t)\not\in \Vars$}} \\[2ex]
...
%% &  & %%? \multicolumn{2}{l|}{{\footnotesize \ \ \ - $w(t)\not\in \Vars$}} 
%% \\[1ex]
%% \prule{t\Incl s, \GSD,p\Incl t}{t\Incl s,\GSD,p\Incl s}
%%  ($\Rsr$) & & \\[2ex]
%%   {\footnotesize \ \ \ - $s,t\not\in\Vars$} & & 
%% \\[2ex]
%% %
...
%$\der{NEQ_4}DS\ \impl\ \der{NEQ_4}{D^*}S$ and $D^*$ contains no applications of 
...
%y=x, x\Incl y,\GSD,y\Incl y & \rabove{(=_{s})} \cl
%y=x, x\Incl y,\GSD,x\Incl x & \rabove{(=_{s})} \cl
...
%% \[
%% s\not\in\Vars\ \ \ \begin{array}{rl}
%% s\Incl y, x\Incl y,\GSD,x=y&  \\ \cline{1-1}
%% s\Incl y, x\Incl s,\GSD,x=y & \rabove{(\Incl_a^*)}
%% \end{array}
%% \conv
%% \begin{array}{rl}
%% x\Incl y, s\Incl y, x\Incl s,\GSD,x=y&  \\ \cline{1-1}
%% s\Incl y, s\Incl y, x\Incl s,\GSD,x=y & \rabove{(\Incl_{ar})}
%% \end{array}
%% \]
Consider the rule
...
y=y, x\Incl y, z\Incl x,\GSD & \rabove{(\Incl_{ar})} %\cl
%     x\Incl y, z\Incl x,\GSD & \rabove{(cut_{x=})}
}
\conv
\prar{
...
y=y, x\Incl y, z\Incl f(x),\GSD & \rabove{(\Incl_{ar})} %\cl
%     x\Incl y, z\Incl f(x),\GSD & \rabove{(cut_{x=})}
}
\conv
\prar{
