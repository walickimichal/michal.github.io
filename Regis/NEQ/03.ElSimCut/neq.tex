...
transformations leading to an equivalent calculus without the general ($cut$)
rule. The specific axioms are replaced by the {\em specific-cut} rules. The paper
studies also the situation when no specific axioms are present, showing that in this
case no cut rules are needed.}
...
%except for lemma \ref{le:noInclsx} for ($=_{sx}$), they 
do not
...
%
\multicolumn{2}{||c}{{\bf Identity rules}:} & \\[1ex]
\prule{t=s,\Gamma\Seq\Delta, w(s)\preceq q}{t=s,\Gamma\Seq\Delta, w(t)\preceq q} ($=_s$) & 
\prule{s=t, x\Incl w(s), \Gamma,\Seq\Delta}{s=t, x\Incl
