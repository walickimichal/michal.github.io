...
% and to give the reader a more detailed account of other 
...
%% but we repeat this
%% definition without any reference to relations but include sorts (just
%% this once).
%
...
% M$ as the set $\under M$ or $\PSet(\under M)$. Having the obvious
% embedding of $\under M$ into $\Card M$, we will often treat $\under M$
% as a subset of $\Card M$.  Also, for $X\subseteq\PSet(Y)$, $\under X$
...
% Any operation $f:N\into\PSet(M)$ has a unique pointwise extension to
% the operation $\PSet(N) \into\PSet(M)$, and
...
% For the opposite, assume that $\under\phi$ is mono. Take arbitrary
% $\psi_1\not=\psi_2:T\into M$, i.e.\ $\exists t\in\under
% T:\psi_1(t)\not=\psi_2(t)$. Since $\under\phi$ is mono,
% lemma~\ref{le:charepimono} implies that
% $\under\phi(\under\psi_1(t))\not=\under\phi(\under\psi_2(t))$, that is
% $\Comp{\psi_1}\phi \not=
% \Comp{\psi_2}\phi$. \\[1ex]
%%%%%% WRONG
% As above, $\impl$ follows since the faithful $\GTerms$ reflects epis.
%
% For the opposite, assume that $\under\phi$ is epi. Take arbitrary
% $\psi_1\not=\psi_2:N\into T$, i.e.\ $\exists n\in\under N
% :\psi_1(n)\not=\psi_2(n)$. Since $\under\phi$ is epi,
% lemma~\ref{le:charepimono} implies that $\exists m\in\under
% M:\under\phi(m)=\{n\}$.  But then
% $\under\psi_1(\under\phi(m))\not=\under\psi_2(\under\phi(m))$, i.e.\
% $\Comp\phi{\psi_1}\not=\Comp\phi{\psi_2}$.
\end{Proof}
