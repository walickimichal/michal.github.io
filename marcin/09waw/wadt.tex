...
The paper characterises compositional homomorphims of relational
structures. A detailed study of three categories of such structures --
viewed as multialgebras -- reveals the one with the most desirable
properties.  In addition, we study analogous categories with
...
{\em type} of relation).
%, written $[R_i:s_1\times...\times s_n]\in \Rels$. 
...
%% Typically, the fact that we consider
...
% , i.e.\ for $f:\under A^n\into\under A$
% and $g:\under A^m\into\under A: f(x_1...g(y_1...y_m)...x_n) =
%\setsup_{z\in g(y_{1}...y_{m})} f(x_1...z...x_n)$.  In general, 
Given
...
%
\eq{
%h(x_{11}...x_{1m_{1}}...x_{n1}...x_{nm_{n}})\ \ =\ \ 
...
%$\MASc$, we obtain the following notions: 
we have: $B$ is a {\em
...
% $f^B(\lis b)\po f^A(\lis b)$, $f^B(x)\rpo f^A(x)$).
...
%% For a homomorphism $\phi:B\into A$, let $\phi[B]$ denote the power set
%% of the image of $\phi$ on the underlying set, i.e.\ $\PSet(\phi[\under
%% B])$ where $\phi[\under B]=\{\phi(n):n\in\under B\}$.

% Since $\phi$ maps $\under B\into\under A$, this set is actually a subset of
% the power set of some subset $\phi[\under B]\subseteq \under A$.
...
% We may restrict the carrier by removing some elements, but only those which
% are not returned by any functions applied to some elements which
% remained in the carrier.  
...
%, also $\MAPS$ which we are about to study, 
...
\end{array}
\label{mof}}
% A partial function $f^A:\under A_1 \into \under A_2$,
% undefined for, say second argument value $a\in \under A_2$, could be
% modeled by a multifunction such that $f^{\mof A}(x,a)=\under A_3$,
% i.e., one which is completely undetermined returning possibly any
% value. This way of modelling partiality by nondeterminism seems to
% enable one to handle strictness in a more flexible fashion than the
% dogma underlying the theory of partial algebras that all functions are
% strict.
% Perhaps, it would also correspond better to the
% ``real life'' of computers, where any function results in {\em
% something}, even if it is an unpredictable system error or crash.  
% We do not follow this line because it would simply require development
% of a new approach.  We merely take partial algebras for what they are,
% and comment on the relationships with our results.
% 
...
% \begin{array}{rcl}
% \lis a\in\dom(f^A) & \Iff & f^{\mof A}(\lis a)\not=\es \\
% \lis a\in\dom(f^A) & \impl & f^{\mof A}(\lis a) = \{f^A(\lis a)\}
% \end{array}
% \label{mof}}
...
%between two partial algebras 
is a {\em weak}
homomorphism iff
%it satisfies  $\Comp{f^A}{\phi}\klee\Comp{\phi}{f^B}$, i.e.:
%\vspace*{.8ex} 
$\phi(\dom(f^A)) \subseteq \dom(f^B)$ and 
$\forall \lis a\in \dom(f^A): \phi(f^A(\lis a))= f^B(\clis a\phi())$. 
A weak homomorphism $\phi$ is also:\vspace*{.5ex}

\noindent\begin{tabular}{l@{\hspace*{.2em}}cl} %@{\hspace*{-.1em}}
...
{\em  relative-injective}. `Full injective' is equivalent to `full
{\em and} injective'.}
